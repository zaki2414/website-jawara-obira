// components/profil/VillagePickerMap.tsx
"use client";
...
type VillagePickerMapProps = {
  village: VillageKey | null;
  onSelect: (village: VillageKey) => void;
};

// Peta "telanjang" — cuma bentuk pulau + tombol pilih desa, tanpa panel
// border/judul/hint text (itu sengaja dibuang per permintaan; dulu ada di
// components/kkn/InteractiveMap.tsx, sekarang dipecah: file ini cuma bagian
...
export function VillagePickerMap({ village, onSelect }: VillagePickerMapProps) {
  ...
  useEffect(() => {
    ...
    const regions: VillageKey[] = ["kawasi", "soligi"];
    ...
    regions.forEach((id) => {
      ...
      const handleMouseEnter = () => {
        if (mapRef.current?.dataset.active === id) return;
        gsap.killTweensOf(paths);
        gsap.to(paths, { fill: MAP_THEME.hover, duration: 0.3, ease: "power2.out" });
      };

      const handleMouseLeave = () => {
        if (mapRef.current?.dataset.active === id) return;
        gsap.killTweensOf(paths);
        gsap.to(paths, { fill: MAP_THEME.default, duration: 0.3, ease: "power2.inOut" });
      };
  ...
  useEffect(() => {
    ...
    const kawasiPaths = document.querySelectorAll<SVGPathElement>("#kawasi path");
    const soligiPaths = document.querySelectorAll<SVGPathElement>("#soligi path");
    const allPaths = [...kawasiPaths, ...soligiPaths];

    mapRef.current.dataset.active = village || "";

    if (!village) {
      gsap.killTweensOf(allPaths);
      gsap.to(allPaths, {
        fill: MAP_THEME.default,
        strokeWidth: 2,
        duration: 0.5,
        ease: "power2.inOut",
      });
    ...
    const activeGroup = document.getElementById(village);
    const activePaths = activeGroup?.querySelectorAll<SVGPathElement>("path");
    if (!activePaths) return;

    const inactivePaths = allPaths.filter((p) => !Array.from(activePaths).includes(p));

    gsap.killTweensOf(allPaths);
    gsap.to(inactivePaths, { fill: MAP_THEME.default, strokeWidth: 2, duration: 0.4 });
    gsap.to(activePaths, {
      fill: MAP_THEME.active,
      strokeWidth: 3,
      duration: 0.5,
      ease: "back.out(1.2)",
    });
  ...
  const handleRegionClick = (regionId: VillageKey) => {
    onSelect(regionId);
  };

  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col items-center gap-6">
      <div className="relative w-full overflow-hidden rounded-2xl border-4 border-on-surface bg-aged-paper p-6 hard-shadow-lg sm:p-10">
        {/* "Pelat laut" biru radial di belakang pulau — sebelumnya cuma kertas
            polos, pulau jadi terasa lepas konteks (tidak ada laut di
            sekitarnya). Cincin putus-putus konsentris menggemakan motif
            kompas di HeroSection.tsx tepat di atas section ini, supaya kedua
            section terasa satu tema atlas, bukan dua panel yang tidak nyambung. */}
        ...
        <svg
          ref={mapRef}
          viewBox="0 0 500 760"
          className="relative z-10 h-auto w-full max-h-[55vh] select-none"
        ...
        >
          <title>Desa Kawasi - Klik untuk detail</title>
          <path
        ...
        <g
          ...
          className="cursor-pointer group"
        >
          <title>Desa Soligi - Klik untuk detail</title>
          <path
        ...
        <text
          ...
          y="315"
          textAnchor="middle"
          className="pointer-events-none select-none font-serif font-black italic text-xl transition-colors duration-300"
          fill={village === "kawasi" ? MAP_THEME.default : MAP_THEME.stroke}
        >
          Kawasi
        </text>
        <text
          ...
          textAnchor="middle"
          className="pointer-events-none select-none font-serif font-black italic text-xl transition-colors duration-300"
          fill={village === "soligi" ? MAP_THEME.default : MAP_THEME.stroke}
        >
          Soligi
        </text>
        </svg>
      ...
      {/*<div className="flex gap-2">
        {(Object.keys(VILLAGES_DATA) as VillageKey[]).map((key) => {
          const Icon = VILLAGES_DATA[key].icon;
          return (
            <button
              key={key}
              type="button"
              onClick={() => handleRegionClick(key)}
              aria-pressed={village === key}
              className={`inline-flex items-center gap-2 rounded-full border-2 border-on-surface px-4 py-2 text-label-md font-black uppercase tracking-wide transition-colors hard-shadow-sm hard-shadow-hover press-effect focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
                village === key ? "bg-primary text-on-primary" : "bg-background text-on-surface"
              }`}
            >
              <Icon className="size-4" aria-hidden="true" />
              {VILLAGES_DATA[key].name}
            </button>
          );
        })}
      </div>*/}

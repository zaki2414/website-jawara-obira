...
// Section backdrop full-bleed — solid bg-tertiary-container (BUKAN gradasi
// tipis), beda per halaman (UMKM=primary, Budaya=tertiary, Fauna=primary-
...
export function TogaHeroSection({ totalCount }: TogaHeroSectionProps) {
  const shouldReduceMotion = useReducedMotion();

  return (
    <section className="relative w-full border-b-4 border-on-surface overflow-hidden bg-tertiary-container py-12 md:py-16 px-4 sm:px-6 md:px-8">
      ...
      {/* ─── KIRI: PANEL JUDUL (primary solid) — kebalikan urutan fauna-obi ─── */}
      <motion.div
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
        className="lg:col-span-7 bg-primary text-on-primary border-4 border-on-surface rounded-3xl p-6 md:p-10 hard-shadow-lg relative overflow-hidden flex flex-col justify-center gap-4 group"
      >
        ...
        <div className="absolute -right-8 -bottom-8 w-48 h-48 opacity-[0.05] pointer-events-none transition-transform duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:rotate-45 group-hover:scale-110 z-10">
          <Image src="/Hiasan 3.svg" alt="" aria-hidden="true" fill className="object-contain" />
        </div>
        ...
        <motion.h1
          ...
          className="relative z-10 font-serif text-3xl sm:text-4xl lg:text-5xl font-black leading-tight tracking-tight"
        >
          {TOGA_CONTENT.title.main}
          <br />
          <span className="italic text-tertiary">{TOGA_CONTENT.title.italic}</span>
        </motion.h1>
        ...
        <motion.p
          ...
          className="relative z-10 font-medium text-sm md:text-base leading-relaxed max-w-2xl"
        >
          {TOGA_CONTENT.description}
        </motion.p>
      </motion.div>

      {/* ─── KANAN: BLOK WARNA SOLID (tertiary) — counter tanaman ─── */}
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ type: "spring" as const, stiffness: 160, damping: 15, delay: shouldReduceMotion ? 0 : 0.25 }}
        className="lg:col-span-5 bg-tertiary text-on-tertiary border-4 border-on-surface p-6 md:p-8 rounded-3xl hard-shadow-lg relative overflow-hidden flex flex-col justify-between gap-5"
      >
        ...
        <div className="relative z-10">
          ...
          <div className="text-label-sm font-black uppercase tracking-wider border-t border-on-tertiary pt-2">
            Tanaman Terdokumentasi
          </div>
        </div>

        <p className="relative z-10 text-sm font-semibold leading-relaxed border-t-2 border-dashed border-on-tertiary pt-4">
          Setiap spesimen mencatat khasiat kesehatan dan resep racikan turun-temurun masyarakat Pulau Obi.
        </p>
      </motion.div>
      </div>
    </section>

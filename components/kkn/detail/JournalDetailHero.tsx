...
export function JournalDetailHero({
  title,
  coverImage,
  activityDate,
  villageName,
        ...
        <div className="absolute inset-0 flex items-end p-6 md:p-10">
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              {activityDate && (
                <span className="inline-flex items-center gap-1.5 text-label-sm font-black uppercase tracking-widest px-3 py-1 rounded-md border-2 border-on-surface bg-background text-on-surface">
                  <Calendar className="w-3.5 h-3.5 text-primary" /> {activityDate}
                </span>
              )}
              {villageName && (
                <span
                ...
                >
                  <MapPin className="w-3.5 h-3.5" /> {villageName}
                </span>
              )}
            </div>

...
const villagesData: Record<string, VillageData> = {
  kawasi: {
    ...
    slug: "kawasi",
    description:
      "Desa Kawasi dikenal sebagai pusat pertanian dan perkebunan di Pulau Obi. Memiliki potensi unggulan dalam pengembangan tanaman obat keluarga (TOGA) dan pertanian organik.",
    population: "± 1.250 Jiwa",
    area: "15.4 km²",
    highlight: "Sentra Pertanian & TOGA",
  },
  soligi: {
    id: "soligi",
    name: "Desa Soligi",
    slug: "soligi",
    description:
      "Desa Soligi merupakan desa pesisir yang kaya akan potensi bahari. Masyarakatnya banyak yang bermatapencaharian sebagai nelayan dan pengolah hasil laut, dengan keindahan pantai yang masih alami.",
    population: "± 980 Jiwa",
    area: "12.8 km²",
    highlight: "Potensi Bahari & Wisata Pantai",
  },
};
...
export default function InteractiveMap() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const kawasiPaths = document.querySelectorAll("#kawasi path");
      const soligiPaths = document.querySelectorAll("#soligi path");

      // Helper untuk hover animation
      const handleHover = (paths: NodeListOf<Element>, isEnter: boolean) => {
        const color = isEnter ? "#0ea5e9" : "#cbd5e1";
        gsap.to(paths, {
          fill: color,
          duration: 0.3,
          ease: "power2.out",
      ...
      if (kawasiPaths.length > 0) {
        kawasiPaths.forEach((path) => {
          path.addEventListener("mouseenter", () => handleHover(kawasiPaths, true));
          path.addEventListener("mouseleave", () => handleHover(kawasiPaths, false));
        });
      }

      if (soligiPaths.length > 0) {
        soligiPaths.forEach((path) => {
          path.addEventListener("mouseenter", () => handleHover(soligiPaths, true));
          path.addEventListener("mouseleave", () => handleHover(soligiPaths, false));
        });
      }
    }, mapRef);
  ...
  const handleRegionClick = (regionId: string) => {
    const ctx = gsap.context(() => {
      // Reset semua area ke warna default
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8">
      {/* AREA PETA SVG */}
      <div className="relative bg-sand-50 rounded-2xl p-6 md:p-10 border border-sand-200 shadow-inner">
        <h3 className="text-center font-serif text-xl font-bold text-ocean-800 mb-6">
          Peta Wilayah Desa Kawasi & Soligi
        </h3>
        ...
        <p className="text-center text-sm text-gray-500 mt-4">
          💡 Klik pada area peta untuk melihat detail informasi desa.
        </p>
      </div>
      ...
      {selectedData && (
        <div
          ref={infoRef}
          className="bg-white rounded-2xl p-6 md:p-8 border border-ocean-100 shadow-lg"
        >
          <div className="flex flex-col md:flex-row gap-6">
            {/* Thumbnail / Icon Desa */}
            <div className="w-full md:w-1/3 shrink-0">
              <div className="bg-ocean-50 rounded-xl p-6 text-center h-full flex flex-col justify-center items-center border border-ocean-100">
                <span className="text-5xl mb-3">🏘️</span>
                <h2 className="font-serif text-2xl font-bold text-ocean-800">
                  {selectedData.name}
                </h2>
                <span className="inline-block mt-2 px-3 py-1 bg-tropic-100 text-tropic-700 text-xs font-bold rounded-full">

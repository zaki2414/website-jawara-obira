...
type KKNSectionBannerAction = {
  href: string;
  label: string;
  icon: LucideIcon;
};

type KKNSectionBannerProps = {
  crumbs: Crumb[];
  title: string;
  subtitle: string;
  badgeLabel: string;
  badgeIcon: LucideIcon;
  action?: KKNSectionBannerAction;
};

// Header section KKN (Tim, Jurnal, dst) — struktur SAMA PERSIS dengan
// KKNHubHero.tsx (panel aged-paper + AdminOrnaments + Badge di kanan atas +
// judul/CTA), supaya identitas KKN hub dan halaman-halaman di dalamnya
// konsisten. Bedanya cuma baris atas: hub pakai teks eyebrow statis, di sini
// breadcrumb (karena halaman-halaman ini 2 level lebih dalam). Dipakai
// bersama oleh tim/jurnal (list + [id]) — parameternya beda, struktur identik.
// (Sebelumnya bernama KKNTeamBanner.tsx, digeneralisasi supaya tidak
// diduplikasi saat Jurnal butuh treatment yang sama.)
export function KKNSectionBanner({
  crumbs,
  title,
  subtitle,
  badgeLabel,

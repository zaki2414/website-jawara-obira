...
type BudayaSectionBannerAction = {
  href: string;
  label: string;
  icon: LucideIcon;
};

type BudayaSectionBannerProps = {
  crumbs: Crumb[];
  title: string;
  subtitle: string;
  badgeLabel: string;
  badgeIcon: LucideIcon;
  action?: BudayaSectionBannerAction;
};

// Header section Budaya (list + form) — struktur SAMA PERSIS dengan
// components/admin/kkn/KKNSectionBanner.tsx (panel aged-paper + AdminOrnaments
// + Badge di kanan atas + judul/CTA), tapi variant ornamen "budaya" (tertiary)
// dan Badge "solid-tertiary" (kuning/emas) — karena kotak menu "Manajemen
// Budaya" di dashboard admin berwarna tertiary, bukan biru seperti KKN.
// Sengaja file terpisah (bukan reuse KKNSectionBanner) supaya domain budaya
// tidak bergantung ke modul bernama "KKN".
export function BudayaSectionBanner({
  crumbs,
  title,
  subtitle,
  badgeLabel,

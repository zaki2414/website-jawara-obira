...
type PetaSectionBannerAction = {
  href: string;
  label: string;
  icon: LucideIcon;
};

type PetaSectionBannerProps = {
  crumbs: Crumb[];
  title: string;
  subtitle: string;
  badgeLabel: string;
  badgeIcon: LucideIcon;
  action?: PetaSectionBannerAction;
};

// Header section Peta Fasilitas (list + form) — struktur SAMA PERSIS dengan
// components/admin/budaya/BudayaSectionBanner.tsx, tapi variant ornamen
// "budaya" (tertiary, direuse apa adanya) dan Badge "solid-tertiary" —
// karena kotak menu "Peta Fasilitas" di dashboard admin berwarna tertiary,
// satu keluarga aksen dengan KKN Hub/Galeri Foto. Sengaja file terpisah
// (bukan reuse BudayaSectionBanner) supaya domain peta tidak bergantung ke
// modul bernama domain lain.
export function PetaSectionBanner({
  crumbs,
  title,
  subtitle,
  badgeLabel,

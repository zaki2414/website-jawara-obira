...
export default async function NewsFeed() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="font-serif text-3xl font-bold text-ocean-800 mb-8">
        Berita Terkini
      </h1>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {news?.map((item: any) => (
          ...
          >
            ...
            <div className="p-4">
              <span className="text-xs font-semibold text-tropic-600 uppercase bg-tropic-50 rounded">
                {item.villages?.name}
              </span>
              <h2 className="font-serif text-lg font-bold text-ocean-800 mt-1 group-hover:text-ocean-600 line-clamp-2">
                {item.title}
              </h2>

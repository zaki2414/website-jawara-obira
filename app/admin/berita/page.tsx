  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="font-serif text-2xl font-bold text-ocean-800">📰 Manajemen Berita</h1>
        <Link href="/admin/berita/new" className="px-4 py-2 bg-ocean-600 text-black rounded-lg hover:bg-ocean-700 transition">+ Tambah Berita</Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-sand-200 overflow-hidden">
          ...
          <tbody className="divide-y divide-sand-100">
            {news?.map((item: any) => (
              <tr key={item.id} className="hover:bg-sand-50">
                <td className="p-4">
                  <Link href={`/berita/${item.villages?.slug || 'kawasi'}/${item.slug}`} className="text-ocean-600 hover:underline font-medium">
                    {item.title}
                  </Link>
                </td>
                ...
                <td className="p-4 text-right space-x-2">
                  <Link href={`/admin/berita/${item.id}`} className="text-tropic-600 hover:text-tropic-700 text-sm font-medium">Edit</Link>
        ...
        {(!news || news.length === 0) && <p className="p-6 text-center text-gray-500">Belum ada berita.</p>}
      </div>
    </div>
  )
}

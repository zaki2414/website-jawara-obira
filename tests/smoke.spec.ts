...
test("/admin menolak pengunjung yang belum login", async ({ page }) => {
  // DISABLE_ADMIN_AUTH=true adalah bypass khusus dev (lihat CLAUDE.md §6.2).
  // Kalau flag itu aktif, /admin memang SENGAJA terbuka tanpa login, jadi
  // menguji redirect di sini akan gagal karena alasan yang salah.
  test.skip(
    process.env.DISABLE_ADMIN_AUTH === "true",
    "DISABLE_ADMIN_AUTH aktif — guard sengaja dimatikan untuk dev lokal",
  );
